...
int main(int argc, char** argv){
    ...
    float arr_size = Nx * Ny;
    float* rho = calloc(Nx * Ny, sizeof(float));
    ...
    const float common_multiplier = (1 / (5 / (hx * hx) + 5 / (hy * hy)));
    const float A = common_multiplier * 0.5f * (5 / (hx * hx) - 1 / (hy * hy));
    const float B = common_multiplier * 0.5f * (5 / (hy * hy) - 1 / (hx * hx));
    const float C = 0.05f;

    float* D = calloc(Nx * Ny, sizeof(float));
    for (int i = 0; i < Ny; ++i){
        for (int j = 0; j < Nx; ++j){
